...
def hent_sesonger():
    ...
    cursor = conn.cursor()
    rows = cursor.execute("""
        SELECT DISTINCT season FROM players
        UNION
        SELECT DISTINCT season FROM goalies
...
@router.get("/spillere")
def hent_spillere(divisjon: str = None, sesong: str = None):
    conn = get_connection()
    cursor = conn.cursor()
...
@router.get("/keepere")
def hent_keepere(divisjon: str = None, sesong: str = None):
    conn = get_connection()
    cursor = conn.cursor()
